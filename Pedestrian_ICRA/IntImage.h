...
public:
    ...
    bool Load(IplImage *img, const char channel='I');
...
template<class T>
bool IntImage<T>::Load(IplImage *img,const char channel)
{
    IplImage* img2;
    ...
    SetSize(img2->height,img2->width);
    for(int i=0,ih=img2->height,iw=img2->width;i<ih;i++)
    {
        T* pdata = p[i];
        unsigned char* pimg = reinterpret_cast<unsigned char*>(img2->imageData+img2->widthStep*i);
        for(int j=0;j<iw;j++) pdata[j] = pimg[j];
    }
    cvReleaseImage(&img2);
