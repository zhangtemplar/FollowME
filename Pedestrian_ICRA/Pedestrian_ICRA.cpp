...
bool CPedestrainRect::Intersect(CPedestrainRect& result,const CPedestrainRect& rect2) const
{
...
bool CPedestrainRect::Union(CPedestrainRect& result,const CPedestrainRect& rect2) const
{
    if(Empty())
    {
        if(rect2.Empty())
...
void PostProcess(std::vector<CPedestrainRect>& result,const int combine_min)
{
    std::vector<CPedestrainRect> res1;
    std::vector<CPedestrainRect> resmax;
    std::vector<int> res2;
    bool yet;
    CPedestrainRect rectInter;

    for(unsigned int i=0,size_i=result.size();i<size_i;i++)
    {
        yet = false;
        CPedestrainRect& result_i = result[i];
        for(unsigned int j=0,size_r=res1.size();j<size_r;j++)
        {
            CPedestrainRect& resmax_j = resmax[j];
...
void RemoveCoveredRectangles(std::vector<CPedestrainRect>& result)
{
	std::vector<bool> covered;
	covered.resize(result.size());
	std::fill(covered.begin(),covered.end(),false);
	CPedestrainRect inter;
	...
	std::vector<CPedestrainRect> newresult;
	for(unsigned int i=0;i<result.size();i++)
		if(covered[i]==false)
...
int DetectionScanner::FastScan(IntImage<double>& original,std::vector<CPedestrainRect>& results,const int stepsize)
{
