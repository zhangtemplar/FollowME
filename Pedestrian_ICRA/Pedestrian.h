...
// my replacement for the CPedestrainRect class in MS MFC -- only provides a limited number of functions
class CPedestrainRect
{
    public:
        double left;
        double top;
    ...
    // Intersect and Union of two rectangles, both function should be able to run when &result==this
        bool Intersect(CPedestrainRect& result,const CPedestrainRect& rect2) const;
        bool Union(CPedestrainRect& result,const CPedestrainRect& rect2) const;
};
...
private:
	...
	void ResizeImage();
public:
	int Scan(IntImage<double>& original,std::vector<CPedestrainRect>& results,const int stepsize,const int round,std::ofstream* out,const int upper_bound);
	int FastScan(IntImage<double>& original,std::vector<CPedestrainRect>& results,const int stepsize);
